...
const messageText = `
╔═════════════════════╗
 ❰ ⚠️ ❱ *مستــخدم محظــور!* ❰ ⚠️ ❱
—◉ *Notice ${messageNumber}/3 (Total: 3)*
—◉ Reason ${user.bannedReason ? `\n*:* ${user.bannedReason}` : '*Reason:* Unspecified'}
—◉ *إذا كنت تعتقد أن هذا خطأ ولديك دليل، فيمكنك الاتصال بمالك الروبوت لاستئناف التعليق.* —◉ *التواصل مع مالكه:* wa.me/201005603546
╚═════════════════════╝
               `.trim();
              m.reply(messageText);
        ...
        try {
          ...
          if (m.limit) {
            m.reply('* 💎 ســوف تستخــدم' + +m.limit + ' diamond(s) (limits).*');
          }
        }
        break;
      }
