...
const handler = async (m, { conn, args }) => {
  ...
  if (feature === 'تجهيز') {
    if (gameData) {
      return conn.reply(m.chat, '⚠️ *لقد بدأت اللعبة.*', m);
    }
    chessData.gameData = { status: 'waiting', black: null, white: null };
    return conn.reply(m.chat, '🎮 *تبدأ لعبة الشطرنج.*\nفي انتظار انضمام لاعبين آخرين.', m);
  }

  if (feature === 'انضمام') {
    const senderId = m.sender;
    if (players.includes(senderId)) {
      return conn.reply(m.chat, '🙅‍♂️ *لقد انضممت إلى اللعبة.*', m);
    }
    if (!gameData || gameData.status !== 'waiting') {
      return conn.reply(m.chat, '⚠️ *لا توجد مباريات شطرنج معلقة.*', m);
    }
    if (players.length >= 2) {
      return conn.reply(m.chat, '👥 *هناك ما يكفي من اللاعبين.*\nتبدأ اللعبة تلقائيًا.', m);
    }
    players.push(senderId);
    hasJoined.push(senderId);
    if (players.length === 2) {
      gameData.status = 'استعد';
  ...
  if (feature === 'ابدا') {
    if (gameData.status !== 'استعد') {
      return conn.reply(m.chat, '⚠️ *لا يمكن بدء اللعبة. انتظر حتى ينضم لاعبان.*', m);
    }
    gameData.status = 'العب';
    const senderId = m.sender;
    if (players.length === 2) {
      const fen = 'rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1';
      chessData.fen = fen;
      const encodedFen = encodeURIComponent(fen);
      const giliran = `🎲 *دور:* أبيض @${gameData.white.split('@')[0]}`;
